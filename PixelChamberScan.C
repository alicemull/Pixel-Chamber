...
void PixelChamberScan(int myevent=0)
{
  ...
  //  TFile *file1 = new TFile("B2_150Planes.root");
  TFile *file1 = new TFile("inelasticTree_180Planes.root");
  ...
  double px, py, pz;
  //
  //  ntupleDigit->SetBranchAddress("EventID_D_in",&EventID_D); 	
  ...
  ntupleDigit->SetBranchAddress("px", &px);
  ntupleDigit->SetBranchAddress("py", &py);
  ntupleDigit->SetBranchAddress("pz", &pz);
 
  int nentries = ntupleDigit->GetEntries();
  TH1F *h_cluster_n[1000];
  int nMC[1000]={0};
  ...
  Datapoint *pt;
	...
	for (int ievent=myevent; ievent<myevent+1; ievent++)
	{
	  ...
	  event[ievent]->fitClusters(0);	//****************
		...
		event[ievent]->mergeCompatibleClusters2();//**********
		//event[ievent]->fitClusters(0);	
  ...
  for (int i=myevent; i<myevent+1; i++) 
    {
      h_cluster_n[i] = new TH1F(Form("h_cluster_n%d",i),"cluster_n",200,0.,199.);
  ...
  for (int i=69; i<myevent+1; i++) 
    {
      for (int j = 0; j <= h_cluster_n[i]->GetNbinsX(); j++)
    {
      if (h_cluster_n[i]->GetBinContent(j)>=0)
        nMC[i]++;
    }
        
      int tr_n = event[i]->getClustersNumber();
      std::cout << "Event: " << i << std::endl;
      std::cout << "Number of reconstructed tracks = " << tr_n << std::endl;
      std::cout << "Number of MC clusters = "<< nMC[i] 
      << std::endl;
