...
bool Cluster::isCompatible(Cluster *cluster)
{
	...
	Datapoint *p = (Datapoint*) fnp->At(0);

	double d = this->DataPointDistanceFromLine(p);
	printf("scalar product = %f distance = %f\n",scalarproduct,d);
	  //	if( fabs(scalarproduct) > MIN_COS && this->pLineDistance(ParFit) <= MIN_DIST){
	if( fabs(scalarproduct) > MIN_COS && d <= MIN_DIST){
			return true;
			printf("merging done\n");
	}
	//
...
double Cluster::isCompatible2(Cluster *cluster)
{
	//
  double FracCompatible = 0.;
  TClonesArray *fnp = cluster->getPointsObj();
  Datapoint *p;
  double d = 0.;
  int nPCompatible = 0;
  for(int m = 0; m < fnp->GetEntries(); m++) {
  ...
  FracCompatible = 1.*nPCompatible / fnp->GetEntries();
  return FracCompatible;
...
void Cluster::ViewClusterFit()
{
	...
	TH2F *h_XY=new TH2F("h_coord_XY","Coordinates of Pixel Centre", 512*2 , -7.68*2, 7.68*2, STACK , -STACK*0.05/2., STACK*0.05/2.);
	TH2F *h_YZ=new TH2F("h_coord_YZ","Coordinates of Pixel Centre", STACK , -STACK*0.05/2., STACK*0.05/2., 512 , -7.68, 7.68);
  ...
  for (int l = 0; l < fTracklets->GetEntries(); l++) {
	...
	for (int l = 0; l < fTracklets->GetEntries(); l++)
	{
		...
		while (m < fTracklets->GetEntries())
		{
			...
			//
			printf("now comparing cluster n. %d with cluster n. %d n tracklets=%d\n",l,m,fTracklets->GetEntries());
  ...
  for (int l = 0; l < fTracklets->GetEntries(); l++) {
	...
	for (int l = 0; l < fTracklets->GetEntries(); l++)
	{
		...
		while (m < fTracklets->GetEntries())
		{
		  ...
		  printf("now comparing cluster n. %d with cluster n. %d\n",l,m);
		  printf("now comparing cluster n. %d with cluster n. %d\n",trackl->getClusterID(),trackm->getClusterID());
		  if (trackl->getReducedChi2()<MAX_CHI2 && trackl->isCompatible2(trackm) > 0.5)
		    {
		      //
		      trackl->merge(trackm);
		      // make graph
...
void PixelChamberEvent::CheckNoisePoints(){
  ...
  for (int l = 0; l < fTracklets->GetEntries(); l++) {
   Cluster *trackl = (Cluster*)(fTracklets->At(l));
   if(trackl->getReducedChi2()>MAX_CHI2) continue;
   int m = 0;
   double d, d1, d2;
   double *xyz;
...
void PixelChamberEvent::ViewClusters()
{
  ...
  TH2F *h_coord_nXY = new TH2F("h_coord_nXY","XY projection", 512*2 , -7.68*2, 7.68*2, STACK , -STACK*0.05/2., STACK*0.05/2.);
  TH2F *h_coord_nYZ = new TH2F("h_coord_nYZ","YZ projection", STACK , -STACK*0.05/2., STACK*0.05/2., 512 , -7.68, 7.68);
  ...
  for(int i=0; i< nb; i++)
    {
      ...
      h_array_XY[i]=new TH2F(Form("h_coord_XY%d",i),"XY projection", 512*2 , -7.68*2, 7.68*2, STACK , -STACK*0.05/2., STACK*0.05/2.);
      h_array_YZ[i]=new TH2F(Form("h_coord_YZ%d",i),"YZ projection", STACK , -STACK*0.05/2., STACK*0.05/2., 512 , -7.68, 7.68);
  ...
  /* TCanvas *c1 =new TCanvas("c1","",1000,1000);
   ...
   h_coord_n->GetZaxis()->SetTitle("y[mm]");
   h_coord_n->GetYaxis()->SetTitle("z[mm]");
